...
cmd({
  ...
  category: "download",
  filename: __filename
}, async (rush, mek, m, { from, q, sender, reply }) => {
...
"*Search Results:*\n";
  ...
  text += `\n*Reply with your movie number (1-${searchResults.length})*`;
  reply(text);
});
...
await rush.sendMessage(
        ...
        {
          image: {
            url: "https://github.com/rush1617/RUSH-TD/blob/main/images/movie%20(2).png?raw=true",
          },
          text,
        },
        { quoted: mek }
...
cmd({
...
}, async (rush, mek, m, { body, sender, reply, from }) => {
  await rush.sendMessage(from, { react: { text: "✅", key: m.key } });
  ...
  if (metadata.thumbnail) {
    await rush.sendMessage(from, { image: { url: metadata.thumbnail }, caption: msg }, { quoted: mek });
  } else {
    await rush.sendMessage(from, { text: msg }, { quoted: mek });
  }
  ...
  qualityMsg += `\n*Please Reply with quality number to receive the movie as a document.*`;
  await rush.sendMessage(from, { text: qualityMsg }, { quoted: mek });
});
...
cmd({
  ...
  try {
    ...
    await rush.sendMessage(from, {
      ...
      caption: `*🎬 ${movie.metadata.title}*\n*📊 Quality:* ${selectedLink.quality}\n*💾 Size:* ${selectedLink.size}\n\n✅ *Thank you for using RUSH-TD! Enjoy your movie* 🎬💖`
    }, { quoted: mek });
  } catch (error) {
